        ...
        <div className='w-full'>
            <div className='bg-[#5271ff] w-full text-white uppercase font-semibold relative'>
                <div className='px-8 flex justify-between items-center relative h-[50px]'>
                    ...
                    <div className='flex-wrap hidden lg:flex'>
                        <Link className={`px-6 font-medium py-[13px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'}>
                            Home
                        </Link>
                        {
                            data.map((c, i) => (
                                <Link
                                    key={i}
                                    className={`px-6 font-medium py-[13px] ${path === `/${c.name.toLowerCase()}` ? 'bg-[#00000026]' : ''}`}
...

            {
                ...
                <div className='flex flex-wrap lg:hidden py-2 px-[30px]'>
                    <Link className={`px-4 font-medium py-[5px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href='/'>
                        Home
                    </Link>
                    {
                        data.map((c, i) => (
                            <Link
                                key={i}
                                className={`px-4 font-medium py-[5px] ${path === `/${c.name.toLowerCase()}` ? 'bg-[#00000026]' : ''}`}
                                href={`/${c.name.toLowerCase()}`}
                            >
                                {c.name}
                            </Link>
